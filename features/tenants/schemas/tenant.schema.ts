...
// Kept separate from tenantSchema below: Zod v4 doesn't allow `.partial()`
// on a schema that already carries a `.refine()`, but updateTenant needs a
// partial variant for single-field updates.
...
export const tenantSchema = tenantObjectSchema.refine(
  (data) => !data.leaseEnd || data.leaseEnd > data.leaseStart,
  {
...
// Used by updateTenant. Only re-checks leaseEnd > leaseStart when both are
// present in the same partial payload — a partial update touching just one
// of the two can't validate against the other without loading the existing
// record, which updateTenant doesn't do.
export const tenantUpdateSchema = tenantObjectSchema.partial().refine(
  (data) => !data.leaseEnd || !data.leaseStart || data.leaseEnd > data.leaseStart,
  {
    error: 'La date de fin de bail doit être postérieure à la date de début.',
    path: ['leaseEnd'],
  }
